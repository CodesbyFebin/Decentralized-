...
interface Props {
  ...
  /** Source preselected from the Deploy overview (GitHub, Docker, Upload, template id...). */
  initialSource?: string;
}

export const DeployWizard: React.FC<Props> = ({ onNavigate, initialSource }) => {
      ...
      <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
        <div>
          <div className="text-xs font-mono text-slate-400 flex items-center gap-2">
            <span onClick={() => onNavigate('deploy')} className="hover:text-white cursor-pointer">
              Deploy
            </span>
            <span>&gt;</span>
            <span className="text-cyan-400">New Deployment{initialSource ? ` · ${initialSource}` : ''}</span>
          </div>
