...
| Join invites | `AddNodeView` | `/invites`, `POST /invites/:nonce/revoke` | `GET /api/v1/invites`, `POST /api/v1/invites/{nonce}/revoke` (api.admin) | **LIVE** | State (ACTIVE/USED/REVOKED/EXPIRED) decided by the control plane. Tokens are created only by `dh node invite` (root key); the console never holds them. |
| Validation records | `EvidenceView`, `EvidenceDetailView`, Dashboard | `/evidence/records…` | files under `DH_EVIDENCE_DIR`; `dh evidence verify` via `DH_CLI` | **LIVE** (UNAVAILABLE when not configured) | Outcome as sealed; FAIL kept visible. "Verified here" only after the verifier actually ran. |
| Refused messages | `ActivityView` → Refused | `/rejections` | `view.rejections` (replicated, newest 100) | **LIVE** | The claimed sender is what the message said; a refusal proves nothing about who sent it. |
| Operations | `OperationsView` | `/operations` | derived from replica rows, host lifecycle and volume state | **DERIVED** | The platform has no Operation resource; no percentages. |
| Attention required | Dashboard | `/overview` (`attention`) | derived from the snapshot | **DERIVED** | Every item names its source record. |
